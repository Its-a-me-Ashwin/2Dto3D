...
class ArucoSingleTracker():
    ...
    def trackImage (self,frame):
        found = False
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        corners, ids, _ = aruco.detectMarkers(image=gray, dictionary=self._aruco_dict, 
        ...
        if not ids is None and self.id_to_find in ids[0]:
            ...
            return (    
                        [   
                            tvec[0], tvec[1], tvec[2],          # x,y,z      0,0,0
                            roll_marker,pitch_marker,yaw_marker # Marker WRT Camera
                        ],
                        [
    ...
    def track(self, loop=True, verbose=False, show_video=None):
        self._cap = cv2.VideoCapture(0)
        self._cap.set(cv2.CAP_PROP_FRAME_WIDTH, self.camera_size[0])
        self._cap.set(cv2.CAP_PROP_FRAME_HEIGHT, self.camera_size[1])
        self._kill = False
